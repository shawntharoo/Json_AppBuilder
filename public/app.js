var app = angular.module('accountSecurityQuickstart', ['ngCookies']);
...
app.controller('PhoneVerificationController', function ($scope, $http, $window, $timeout, userPersistenceService) {
    ...
    $scope.init = function() {
        if(userPersistenceService.getCookieData() == 'loggedIn'){
    ...
    $scope.verifyToken = function () {
        $http.post('/api/verification/verify', $scope.setup)
            .success(function (data, status, headers, config) {
                ...
                userPersistenceService.setCookieData("loggedIn");
                $window.location.href = $window.location.origin + "/verified";
            })
            .error(function (data, status, headers, config) {
